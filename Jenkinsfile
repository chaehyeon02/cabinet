    ...
    stages {
        ...
        stage('Build and Run Docker Image') {
            steps {
                script {
                    // Docker 이미지 빌드
                    sh 'docker build -t lch125/cabinet_notices:latest -f Dockerfile .'

                    // Docker 컨테이너 실행 (백그라운드에서 실행)
                    sh 'docker run -d -p 3002:3000 --name cabinet_notices lch125/cabinet_notices:latest'
                }
            }
        }
            ...
            steps {
                script {
                        docker.withRegistry('https://index.docker.io/v1/', 'lch125') {
                            sh "docker push lch125/cabinet_notices"
                        }
